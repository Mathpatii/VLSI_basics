create_clock -name clk -period 2 -waveform {0 1} [get_ports "clk"]
set_clock_transition -rise 0.1 [get_clocks "clk"]
set_clock_transition -fall 0.1 [get_clocks "clk"]
set_clock_uncertainty 0.01 [get_ports "clk"]
set_input_transition 0.12 [all_inputs]
set_input_delay -max 0.8 [get_ports "A"] -clock [get_clocks "clk"]
set_input_delay -max 0.8 [get_ports "B"] -clock [get_clocks "clk"]
set_input_delay -max 0.8 [get_ports "C0"] -clock [get_clocks "clk"]
set_input_delay -max 0.8 [get_ports "S"] -clock [get_clocks "clk"]
set_output_delay -max 0.8 [get_ports "C4"] -clock [get_clocks "clk"]
set_load 0.15 [all_outputs]
set_max_fanout 20.00 [current_design]
